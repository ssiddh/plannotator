...
describe("extractLastRenderedMessage — real-world scenarios", () => {
  test("full conversation: tools + agents + text across a turn", () => {
    const log = buildLog(
      fileSnapshot(),
      userPrompt("Explain the annotation system"),
      assistantText("msg_explain", "Here's how annotations work..."),
      assistantToolUse("msg_explain", "Read"),
      userToolResult("toolu_r1", "file contents"),
      assistantToolUse("msg_tools", "Read"),
      userToolResult("toolu_r2", "more contents"),
      assistantText("msg_summary", "Based on my analysis, the system does X, Y, Z."),
      systemEntry("turn_duration"),
      userPrompt("Now investigate the performance issue"),
      assistantText("msg_perf", "Let me look into that."),
      assistantToolUse("msg_perf", "Agent"),
      userToolResult("toolu_agent", "Agent launched"),
      assistantText("msg_while_agent", "I've launched an agent. The bottleneck is in the parser."),
      assistantToolUse("msg_while_agent", "Grep"),
      userToolResult("toolu_grep", "matches"),
      assistantText("msg_final", "Found the issue. O(n²) in parseMarkdownToBlocks."),
      progress(),
      progress(),
      queueOp(),
      systemEntry(),
      userPrompt("/plannotator-last")
    );
    const entries = parseSessionLog(log);
    const anchor = findAnchorIndex(entries, "/plannotator-last")!;
    const result = extractLastRenderedMessage(entries, anchor);

    expect(result).not.toBeNull();
    // Should get only the LAST message.id with text (msg_final)
    expect(result!.messageId).toBe("msg_final");
    expect(result!.text).toBe("Found the issue. O(n²) in parseMarkdownToBlocks.");
  });

  test("assistant response interrupted by stop hook", () => {
    const log = buildLog(
      userPrompt("Tell me about the project"),
      assistantText("msg_interrupted", "This project is a plan review UI that...", {
        stopReason: null,
      }),
      progress("hook_progress"),
    ...
    const result = extractLastRenderedMessage(entries, anchor);
    expect(result).not.toBeNull();
    expect(result!.text).toContain("plan review UI");
  });

  test("long tool-only sequence then final response", () => {
    const log = buildLog(
      userPrompt("Fix the bug"),
      assistantToolUse("msg_t1", "Read"),
      userToolResult("t1", "code"),
      assistantToolUse("msg_t2", "Grep"),
      userToolResult("t2", "matches"),
      assistantToolUse("msg_t3", "Edit"),
      userToolResult("t3", "edited"),
      assistantToolUse("msg_t4", "Bash"),
      userToolResult("t4", "tests pass"),
      assistantText("msg_done", "Fixed. Missing null check in handler.ts:42."),
      userPrompt("ANCHOR")
    );
    const entries = parseSessionLog(log);
    const anchor = findAnchorIndex(entries, "ANCHOR")!;
    const result = extractLastRenderedMessage(entries, anchor);
    expect(result).not.toBeNull();
    expect(result!.text).toContain("Missing null check");
  });
