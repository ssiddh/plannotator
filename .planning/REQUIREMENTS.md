...
- [x] **ARCH-02**: Upstream file changes limited to single context wrapper in App.tsx
- [x] **ARCH-03**: Handler follows ExternalAnnotationHandler composition pattern
- [x] **ARCH-04**: Fork can rebase on upstream main without merge conflicts in GitHub code
...
- [x] **AUTH-02**: GitHub tokens validated before any PR operations
- [x] **AUTH-03**: ACL users and teams checked against GitHub API before granting access
- [x] **AUTH-04**: Existing OAuth flow preserved (no breaking changes)
...
- [x] **PR-05**: DELETION annotations exported as GitHub code suggestions (```suggestion blocks)
...
- [x] **THREAD-07**: Resolved thread status displayed in Plannotator UI

### Data Model & Sync State

- [x] **DATA-01**: Annotation IDs use stable generation (not ephemeral timestamps)
- [x] **DATA-02**: Bidirectional ID mapping stored (Plannotator annotation ID ↔ GitHub comment ID)
- [x] **DATA-03**: Line mapping reversible (markdown line → block ID + offset)
- [x] **DATA-04**: Sync metadata tracks last sync timestamp and direction
- [x] **DATA-05**: Conflict detection when both sides modified same annotation
